...
function newContact(){
	var arr=[];
	if ($("#contactname").val()!='' && $("#phonenum").val()!=''){

	arr[0]= {namecon: $("#contactname").val(), phone: $("#phonenum").val()};
	setFile(storeC, arr[0]);	
	
	$("#contactname").val('');
	$("#phonenum").val('');
	}
	
	else{alert('Вы что-то не ввели');}
							...
							/*for(var fieldValue in (value=res[field]))
								{*/
									/*switch(fieldValue)
										{
											case 'name':*/
												var namecon=res[field].namecon;/*value[fieldValue].name;*/
											/*case 'phone':*/
												var phone=res[field].phone;/*value[fieldValue].phone;*/
										/*}*/
								/*}*/
		
							$('#contactContent').append( /*mess+=*/'<p><a href="tel:'+phone+'" data-role="button">'+namecon+'</a></p>');
					}
			});
	/*mess+='</ul>'
	document.write(mess);*/
...
function outcanvasg(){	
	...
	getStorage(storeGB, function(res){
			...
			ctx.fillRect(x,y,b,-db);
					...
					'<li style="color:#d35656">Запрашивалась помощь</li>'+
					'</ul>');
